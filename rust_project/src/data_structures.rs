...
impl<T> LockFreeQueue<T> {
    ...
    pub fn enqueue(&self, data: T) {
        ...
        loop {
            ...
            let next = unsafe { (*tail).next.load(AtomicOrdering::Acquire) };
            
            if tail == self.tail.load(AtomicOrdering::Acquire) {
                if next.is_null() {
        ...
        self.tail.compare_exchange_weak(
            self.tail.load(AtomicOrdering::Acquire),
            new_node,
            AtomicOrdering::Release,
            AtomicOrdering::Relaxed,
        ).ok();
    ...
    pub fn dequeue(&self) -> Option<T> {
        loop {
            ...
            let tail = self.tail.load(AtomicOrdering::Acquire);
            let next = unsafe { (*head).next.load(AtomicOrdering::Acquire) };
            
            if head == self.head.load(AtomicOrdering::Acquire) {
                if head == tail {
                    if next.is_null() {
                        return None;
                    }
...
impl<T: Ord + Clone> BST<T> {
                ...
                Ordering::Equal => {
                    *node = match (n.left.take(), n.right.take()) {
                        ...
                        (Some(left), Some(right)) => {
                            ...
                            let min_value = std::mem::replace(&mut min_node.value, n.value.clone());
                            n.value = min_value;
                            n.left = Some(left);
    ...
    fn balance_node(node: &mut Option<Box<BSTNode<T>>>) {
        if let Some(n) = node {
            let balance = Self::get_balance(&Some(Box::new(BSTNode {
                value: n.value.clone(),
                left: n.left.clone(),
                right: n.right.clone(),
                height: n.height,
