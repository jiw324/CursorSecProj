...
class StatisticalAnalyzer {
    ...
    public static <T> Map<T, Double> percentiles(Stream<T> stream, Function<T, Double> mapper, int... percentiles) {
        ...
        for (int percentile : percentiles) {
            ...
            if (!sorted.isEmpty()) {
                result.put((T) ("P" + percentile), sorted.get(index));
            }
        }
...
class DataAnalyticsEngine {
    ...
    private List<AnalyticsResult> performStatisticalAnalysis() {
        ...
        Map<String, Double> percentiles = StatisticalAnalyzer.percentiles(
            salesData.stream(), 
            record -> record.getTotalValue().doubleValue(),
            25, 50, 75, 90, 95
        );
