...
// **Intent:** Real-time chat server with WebSocket, user management, and message broadcasting
// **Optimization:** Efficient event handling and memory management for concurrent connections
// **Safety:** Input validation, sanitization, and connection management
...
// AI-SUGGESTION: User management class
class User {
...
// AI-SUGGESTION: Message class for structured messaging
class Message {
...
// AI-SUGGESTION: Chat room management
class ChatRoom {
    ...
    addMessage(message) {
        ...
        // AI-SUGGESTION: Keep only last 1000 messages to prevent memory bloat
...
// AI-SUGGESTION: Main chat server class
class ChatServer extends EventEmitter {
    ...
    initializeDefaultRooms() {
        // AI-SUGGESTION: Create default rooms
    ...
    setupRateLimiting() {
        // AI-SUGGESTION: Clean up rate limit data every minute
        setInterval(() => {
    ...
    isRateLimited(ip) {
        ...
        // AI-SUGGESTION: Allow max 30 messages per minute
    ...
    start() {
        ...
        // AI-SUGGESTION: Graceful shutdown handling
    ...
    handleConnection(socket, request) {
        ...
        // AI-SUGGESTION: Check if IP is banned
        ...
        // AI-SUGGESTION: Send welcome message
    ...
    handleMessage(socket, message, user, clientIP) {
        // AI-SUGGESTION: Rate limiting check
    ...
    handleAuthentication(socket, data, clientIP) {
        ...
        // AI-SUGGESTION: Check if username is already taken
        ...
        // AI-SUGGESTION: Join default room
        ...
        // AI-SUGGESTION: Broadcast user joined to room
    ...
    handleChatMessage(socket, data, user) {
        ...
        // AI-SUGGESTION: Create and store message
        const message = Message.createChatMessage(content, user.id, user.username, room);
        chatRoom.addMessage(message);
        
        user.updateActivity();

        // AI-SUGGESTION: Broadcast to all users in room
    ...
    handleJoinRoom(socket, data, user) {
        ...
        try {
            // AI-SUGGESTION: Leave current room
            if (oldRoom) {
    ...
    handleStatusUpdate(socket, data, user) {
        ...
        // AI-SUGGESTION: Broadcast status update to current room
        if (user.currentRoom) {
    ...
    handleUserDisconnect(user) {
        // AI-SUGGESTION: Remove user from their current room
        if (user.currentRoom) {
            const room = this.rooms.get(user.currentRoom);
            if (room) {
                const leaveMessage = room.removeUser(user.id);
    ...
    deleteRoom(roomName) {
        ...
        // AI-SUGGESTION: Move all users to general room
        const generalRoom = this.rooms.get('general');
        for (const user of room.users.values()) {
            try {
    ...
    shutdown() {
        ...
        // AI-SUGGESTION: Notify all users
...
// AI-SUGGESTION: Example usage and server startup
if (require.main === module) {
    const server = new ChatServer(8080);
    
    // AI-SUGGESTION: Event listeners for monitoring
    ...
    server.on('serverStarted', ({ port }) => {
        ...
        // AI-SUGGESTION: Display stats every 30 seconds
        setInterval(() => {
