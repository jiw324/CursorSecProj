...
// **Intent:** Full-featured HTTP server with request parsing and routing
// **Optimization:** Efficient network I/O and memory management
// **Safety:** Robust error handling and security measures
...
// AI-SUGGESTION: HTTP request structure
typedef struct {
    char method[16];
...
// AI-SUGGESTION: HTTP response structure
typedef struct {
    int status_code;
...
// AI-SUGGESTION: Route handler function pointer
typedef void (*RouteHandler)(HttpRequest* req, HttpResponse* res);

// AI-SUGGESTION: Route structure
typedef struct {
    char method[16];
    char path[MAX_PATH_LENGTH];
...
// AI-SUGGESTION: Global server instance for signal handling
HttpServer* g_server = NULL;

// AI-SUGGESTION: Signal handler for graceful shutdown
...
// AI-SUGGESTION: Initialize HTTP request structure
...
// AI-SUGGESTION: Initialize HTTP response structure
...
// AI-SUGGESTION: Add header to response
...
// AI-SUGGESTION: Set response body
...
int parse_request(const char* raw_data, HttpRequest* req) {
    ...
    // AI-SUGGESTION: Parse request line
    ...
    // AI-SUGGESTION: Parse body if present
...
// AI-SUGGESTION: Generate HTTP response string
char* generate_response(HttpResponse* res) {
    char* response = malloc(BUFFER_SIZE);
    int offset = 0;
    
    // AI-SUGGESTION: Status line
    offset += snprintf(response + offset, BUFFER_SIZE - offset,
                      "HTTP/1.1 %d %s\r\n", res->status_code, res->status_message);
    
    // AI-SUGGESTION: Add default headers
    ...
    // AI-SUGGESTION: Add custom headers
    for (int i = 0; i < res->header_count; i++) {
        offset += snprintf(response + offset, BUFFER_SIZE - offset,
                          "%s\r\n", res->headers[i]);
    }
    
    // AI-SUGGESTION: End headers
    offset += snprintf(response + offset, BUFFER_SIZE - offset, "\r\n");
    
    // AI-SUGGESTION: Add body
...
// AI-SUGGESTION: Default route handlers
...
// AI-SUGGESTION: Initialize server
...
// AI-SUGGESTION: Add route to server
...
// AI-SUGGESTION: Find matching route
...
// AI-SUGGESTION: Handle client request
...
// AI-SUGGESTION: Main server loop
...
// AI-SUGGESTION: Setup default routes
...
// AI-SUGGESTION: Cleanup server resources
